...
kubectl delete jobs.batch/migration-users-job

echo "Running migration job on kubernetes..."
envsubst < "$SCRIPT_DIR"/../infrastructure/kubernetes/user-service/migration-job.yaml | kubectl apply -f -
JOB_POD=$(kubectl get pods --selector=batch.kubernetes.io/job-name=migration-users-job --output=jsonpath='{.items[*].metadata.name}')

echo "Waiting for job to finish (pod $JOB_POD)"
kubectl wait --for=condition=complete job/migration-users-job
echo "Job completed"
echo "Logs from job:"
kubectl logs job/migration-users-job
